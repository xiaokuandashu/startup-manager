...
const TaskRow: React.FC<TaskRowProps> = ({
  ...
  return (
    <div className={`task-row ${isSelected ? 'selected' : ''}`}>
      ...
      <div className="task-cell">{task.taskType}</div>
      <div className="task-cell">{task.timeType}</div>
      <div className="task-cell">{task.executeTime}</div>
      <div className="task-cell">{task.timeUntilExec}</div>
      ...
      <div className="task-cell task-actions-cell">
        ...
        <div className="more-wrapper">
          <button className="action-link more-btn" ref={moreRef} onClick={() => setMoreOpen(!moreOpen)}>···</button>
          <MoreDropdown
            isOpen={moreOpen}
