...
impl Keyboard {
    ...
    pub(crate) unsafe fn string_from_code(
        ...
        // macOS 14+ will crash (EXC_BREAKPOINT) if TSMGetInputSourceProperty is called on a background thread.
        // We do not need the string name for out recorder, so we just return None to prevent the crash.
        None
    }
}
